<template>
  <div class="dropdown-menu">
    <template v-if="items.length">
      ...
      >
        <button 
          :class="{ 'is-selected': index === selectedIndex, 'has-children': item.children && item.children.length }" 
          @click="selectItem(index)"
        >
          {{ item.label }}
          <span v-if="item.children && item.children.length" class="has-children-icon">›</span>
        </button>
        
        <div 
          v-if="item.children && item.children.length && (hoveredItem === item || (index === selectedIndex && showSubMenuForSelected))" 
          class="sub-menu"
        ...
        >
          <button 
            ...
            :key="childIndex"
            :class="{ 'is-selected': childIndex === selectedChildIndex && selectedParentIndex === index }"
...
interface Props {
  ...
  command: (item: VariableType) => void
}
...
const upHandler = () => {
  if (isNavigatingSubMenu.value && selectedParentIndex.value >= 0) {
    ...
    if (parentItem && parentItem.children && parentItem.children.length > 0) {
      selectedChildIndex.value = (selectedChildIndex.value + parentItem.children.length - 1) % parentItem.children.length
    }
  } else {
...
const downHandler = () => {
  if (isNavigatingSubMenu.value && selectedParentIndex.value >= 0) {
    const parentItem = props.items[selectedParentIndex.value]
    if (parentItem && parentItem.children && parentItem.children.length > 0) {
      selectedChildIndex.value = (selectedChildIndex.value + 1) % parentItem.children.length
    }
  } else {
...
const selectSubItem = (parent: VariableType, child: VariableType) => {
  ...
  props.command(combinedItem as VariableType)
}
